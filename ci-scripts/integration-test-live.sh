...
usage () {
  ...
  echo "-x <boolean> -- Pass this argument if the node has CORS disabled"
...
SKYCOIN_INTEGRATION_TESTS=1 SKYCOIN_INTEGRATION_TEST_MODE=$MODE RPC_ADDR=$RPC_ADDR \
	SKYCOIN_NODE_HOST=$HOST USE_CSRF=$USE_CSRF ALLOW_CORS=$ALLOW_CORS LIVE_DISABLE_NETWORKING=$DISABLE_NETWORKING \
