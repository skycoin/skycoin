...
import (
	...
	"github.com/skycoin/skycoin/src/hardware-wallet/skywallet/usb"
...
type Devicer interface {
	...
	Ping(msg string) (wire.Message, error)
...
func (d *Device) AddressGen(addressN, startIndex uint32, confirmAddress bool, coinType CoinType) (wire.Message, error) {
	...
	defer func() { _ = d.Disconnect() }() //nolint:errcheck
...
// Ping sends a Ping message to the device and returns the response
func (d *Device) Ping(msg string) (wire.Message, error) {
	if err := d.Connect(); err != nil {
		return wire.Message{}, err
	}
	defer func() { _ = d.Disconnect() }() //nolint:errcheck
	...
	return d.Driver.SendToDevice(d.dev, pingChunks)
}
