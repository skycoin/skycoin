...
func CreateRawTx(c GetOutputser, wlt *wallet.Wallet, inAddrs []string, chgAddr string, toAddrs []SendAmount, password []byte) (*coin.Transaction, error) {
	...
	if err := visor.VerifySingleTxnSoftConstraints(*txn, head.Time, inUxsFiltered, params.UserMaxTransactionSize, params.UserBurnFactor, params.UserMaxDropletPrecision); err != nil {
		return nil, err
	}
