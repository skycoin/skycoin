...
func (vs *Visor) NewReadableBlocks(blocks []coin.SignedBlock) (*ReadableBlocks, error) {
	...
	for _, b := range blocks {
		...
		resTxns := make([]ReadableTransaction, 0, len(b.Body.Transactions))
		for _, txt := range b.Body.Transactions {
			t := Transaction{
				Txn:    txt,
				Status: TransactionStatus{BlockSeq: b.Seq()},
			}
			rdt, err := vs.NewReadableTransaction(t)
			if err != nil {
				logger.Errorf("Visor.NewReadableTransaction: failed: %v", err)
				return nil, err
			}
...
func (vs *Visor) NewReadableTransactionInputs(transaction Transaction) ([]ReadableTransactionInput, []string, error){
	...
	for _, inputID := range transaction.Txn.In {
		...
		if err != nil {
			logger.Errorf("Visor.NewReadableTransaction: Visor.GetUxOutByID failed: %v", err)
			return nil, nil, err
		}
		if input == nil {
