...
// VerifySingleTxnSoftHardConstraints provides a mock function with given fields: tx, txn, maxSize, burnFactor, maxDropletPrecision
func (_m *MockBlockchainer) VerifySingleTxnSoftHardConstraints(tx *dbutil.Tx, txn coin.Transaction, maxSize uint32, burnFactor uint32, maxDropletPrecision uint8) error {
	ret := _m.Called(tx, txn, maxSize, burnFactor, maxDropletPrecision)

	var r0 error
	if rf, ok := ret.Get(0).(func(*dbutil.Tx, coin.Transaction, uint32, uint32, uint8) error); ok {
		r0 = rf(tx, txn, maxSize, burnFactor, maxDropletPrecision)
	} else {
		r0 = ret.Error(0)
	}
