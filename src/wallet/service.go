...
func (serv *Service) CreateWallet(wltName string, options Options, scanN uint64, bg BalanceGetter) (*Wallet, error) {
	serv.Lock()
	defer serv.Unlock()
	if !serv.enableWalletAPI {
	...
	return serv.loadWallet(wltName, options, scanN, bg)
}
...
func (serv *Service) loadWallet(wltName string, options Options, scanN uint64, bg BalanceGetter) (*Wallet, error) {
	...
	w, err := NewWalletWithScan(wltName, options, scanN, bg)
	if err != nil {
		return nil, err
	}
