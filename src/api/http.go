...
func newServerMux(c muxConfig, gateway Gatewayer) *http.ServeMux {
	...
	webHandlerV2("/wallet/transaction/sign", forAPISet(walletSignTransactionHandler(gateway, false), []string{EndpointsWallet}))
	...
	webHandlerV2("/transaction/raw", forAPISet(rawTxnHandler(gateway, true), []string{EndpointsRead}))
