		...
		},
		{
			...
			expect: expect{
				meta: map[string]string{
					"label":    "wallet1",
					"filename": "test.wlt",
					"coin":     string(meta.CoinTypeBitcoin),
					"type":     WalletTypeDeterministic,
					"seed":     "testseed123",
				},
				err: nil,
	...
	for _, tc := range tt {
		for _, ct := range crypto.TypesInsecure() {
			...
			t.Run(name, func(t *testing.T) {
				...
				err := w.Unlock(tc.unlockPwd, func(wlt Wallet) error {
...
func TestWalletCollectionAddEntry(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.err, w.(*CollectionWallet).AddEntry(entry.Entry{
				Address: a,
				Public:  p,
				Secret:  tc.secKey,
			}))
	...
	for _, ct := range crypto.TypesInsecure() {
		for _, tc := range cases {
			t.Run(fmt.Sprintf("crypto=%v", ct), func(t *testing.T) {
				...
				err = w.Unlock([]byte("pwd"), func(w Wallet) error {
