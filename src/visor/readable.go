...
// Unreadable converts a ReadableTransaction back to a coin.Transaction
func (r ReadableTransaction) Unreadable() (*coin.Transaction, error) {
	t := coin.Transaction{}
	...
	for i, n := range r.In {
		in[i], err = cipher.SHA256FromHex(n)
		if err != nil {
			return nil, err
		}
	}
