...
Test(api_cli, TestLoadConfig) {
  ...
  GoUint32 errcode;
  GoString_ strConfigValue;
  GoString_ strEnv;
  GoString_ strEnv2;
  GoString strEnvCoinVar = {SKYCOIN_COIN_ENV_VAR, 4};

  GoString strEnvRPCVar = {SKYCOIN_RPC_ENV_VAR, 8};

  GoString strEnvCoinFoo = {SKYCOIN_COIN_FOO, 7};

  GoString strEnvRPCSample = {SKYCOIN_RPC_SAMPLE, 24};

  GoString strWalletDirVar = {SKYCOIN_WALLET_DIR_ENV_VAR, 10};

  GoString strWalletNameVar = {SKYCOIN_WALLET_NAME_ENV_VAR, 11};

  GoString strWalletDirSample = {SKYCOIN_WALLET_DIR_SAMPLE, 13};

  GoString strWalletNameSample = {SKYCOIN_WALLET_NAME_SAMPLE, 7};
  ...
  cr_assert(strcmp(strConfigValue.p, strEnvCoinFoo.p) == 0,
            "SKY_cli_LoadConfig with coin failed");
  SKY_cli_Setenv(strEnvCoinVar, *(GoString *)&strEnv); // Restore previous value
  registerMemCleanup((void *)strConfigValue.p);
  registerMemCleanup((void *)strEnv.p);
  ...
  cr_assert(strcmp(strConfigValue.p, strEnvRPCSample.p) == 0,
            "SKY_cli_LoadConfig with RPCAddress failed");
  SKY_cli_Setenv(strEnvCoinVar, *(GoString *)&strEnv); // Restore previous value
  registerMemCleanup((void *)strConfigValue.p);
  registerMemCleanup((void *)strEnv.p);
  SKY_handle_close((Handle)configHandle);
  ...
  errcode = SKY_cli_Setenv(strWalletDirVar, *(GoString *)&strWalletDirSample);
  ...
  cr_assert(strcmp(strConfigValue.p, SKYCOIN_WALLET_FULL_PATH_SAMPLE) == 0,
            "SKY_cli_LoadConfig with Wallet Dir failed");
  SKY_cli_Setenv(strWalletDirVar,
                 *(GoString *)&strEnv); // Restore previous value
  SKY_cli_Setenv(strWalletNameVar,
                 *(GoString *)&strEnv2); // Restore previous value
  registerMemCleanup((void *)strConfigValue.p);
  registerMemCleanup((void *)strEnv.p);
  registerMemCleanup((void *)strEnv2.p);
  SKY_handle_close((Handle)configHandle);
}
