...
func (d decoder) string() string { // nolint: unused
...
func (e encoder) string(xs string) { // nolint: unused
	x := []byte(xs)
	l := len(x)
	for i := 0; i < l; i++ {
		e.buf[i] = x[i]
...
//v.SetBytes(d.bytes())
func (d decoder) bytes() []byte { // nolint: unused
	l := int(d.uint32()) //pop length
	t := d.buf[:l]
	d.buf = d.buf[l:]
	return t
...
func (e encoder) bytes(x []byte) { // nolint: unused
	l := len(x)
	for i := 0; i < l; i++ {
		e.buf[i] = x[i]
	} //memcpy
...
func (d *decoder) skip(v reflect.Value) { // nolint: unused
	n, _ := datasizeWrite(v)
	d.buf = d.buf[n:]
}
...
*/
func (e *encoder) skip(v reflect.Value) { // nolint: unused
	n, _ := datasizeWrite(v)
