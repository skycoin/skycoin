...
// chbBalanceResponse the response data struct for /api/v2/coinhourbank/balance
type chbBalanceResponse struct {
	Balance uint64
}

// Balance gets balance of a coinhour bank account
// URI: /coinhourbank/balance
// Method: GET
// Args:
// 	address [string]
func chbBalanceHandler(gateway Gatewayer) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		balance, err := gateway.CoinhoursBalance(addr)
		if err != nil {
			wh.Error500(w, err.Error())
			return
		}
