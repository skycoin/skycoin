...
func TestTransactionSignInputs(t *testing.T) {
	...
	require.Nil(t, cipher.VerifySignatureForAddress(a, cipher.AddSHA256(h, tx.In[0]), tx.Sigs[0]))
	require.Nil(t, cipher.VerifySignatureForAddress(a2, cipher.AddSHA256(h, tx.In[1]), tx.Sigs[1]))
	require.NotNil(t, cipher.VerifySignatureForAddress(a, h, tx.Sigs[1]))
	require.NotNil(t, cipher.VerifySignatureForAddress(a2, h, tx.Sigs[0]))
}
