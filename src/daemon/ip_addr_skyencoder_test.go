...
func testSkyencoderIPAddr(t *testing.T, obj *IPAddr) {
...
func TestSkyencoderIPAddr(t *testing.T) {
...
func testSkyencoderIPAddrDecodeErrors(t *testing.T, k int, tag string, obj *IPAddr) {
	isEncodableField := func(f reflect.StructField) bool {
		// Skip unexported fields
		if f.PkgPath != "" {
			return false
...
func TestSkyencoderIPAddrDecodeErrors(t *testing.T) {
	rand := mathrand.New(mathrand.NewSource(time.Now().Unix()))
	n := 10
	...
	for i := 0; i < n; i++ {
		...
		testSkyencoderIPAddrDecodeErrors(t, i, "empty", emptyObj)
		testSkyencoderIPAddrDecodeErrors(t, i, "full", fullObj)
	}
}
