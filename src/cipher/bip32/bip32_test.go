...
func testVectorKeyPairs(t *testing.T, vector testMasterKey) {
	...
	for i, testChildKey := range vector.children {
		t.Run(fmt.Sprintf("%s-%d", vector.seed, i), func(t *testing.T) {
			fmt.Println("--------")
			fmt.Println(i)
			...
			// DEBUG PARSING OF THE EXPECTED KEY
			ppk, err := base58.Decode(testChildKey.privKey)
			require.NoError(t, err)
			xx, err := DeserializePrivate(ppk)
			require.NoError(t, err)
			fmt.Println("parent fingerprint:", hex.EncodeToString(xx.ParentFingerprint))
			fmt.Println("depth:", xx.Depth)
			fmt.Println("childNumber:", xx.ChildNumber())
...
func TestParentPublicChildDerivation(t *testing.T) {
	...
	for _, child := range expectedChildren {
		t.Run(fmt.Sprint(child.pathFragment), func(t *testing.T) {
			...
			pubKey2, err := extendedMasterPrivate.NewPublicChildKey(child.pathFragment)
			require.NoError(t, err)
			require.Equal(t, pubKey, pubKey2)

			privKey, err := extendedMasterPrivate.NewPrivateChildKey(child.pathFragment)
			require.NoError(t, err)
