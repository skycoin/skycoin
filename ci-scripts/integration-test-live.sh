...
usage () {
  ...
  echo "-x <boolean> -- Pass this argument if the node has headercheck disabled"
...
SKYCOIN_INTEGRATION_TESTS=1 SKYCOIN_INTEGRATION_TEST_MODE=$MODE RPC_ADDR=$RPC_ADDR \
	SKYCOIN_NODE_HOST=$HOST USE_CSRF=$USE_CSRF HEADERCHECK=$HEADERCHECK LIVE_DISABLE_NETWORKING=$DISABLE_NETWORKING \
