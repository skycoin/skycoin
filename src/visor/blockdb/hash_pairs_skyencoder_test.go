...
func testSkyencoderHashPairs(t *testing.T, obj *HashPairs) {
	...
	// Encode
	data2, err := encodeHashPairs(obj)
	if err != nil {
		t.Fatalf("encodeHashPairs failed: %v", err)
	}
	if uint64(len(data2)) != n2 {
		t.Fatal("encodeHashPairs produced bytes of unexpected length")
	}
	...
	if err := encodeHashPairsToBuffer(data3, obj); err != nil {
		t.Fatalf("encodeHashPairsToBuffer failed: %v", err)
	}
	...
	var obj4 HashPairs
	n, err := decodeHashPairs(data3, &obj4)
	if err != nil {
		t.Fatalf("decodeHashPairs failed: %v", err)
	}
	...
	if hasOmitEmptyField(&obj4) && omitEmptyLen(&obj4) == 0 {
		...
		if n != n2+4 {
			t.Fatalf("decodeHashPairs bytes read length should be %d, is %d", n2+4, n)
		}
	} else {
		if n != n2 {
			t.Fatalf("decodeHashPairs bytes read length should be %d, is %d", n2, n)
		}
	}
	...
	// DecodeExact
	var obj5 HashPairs
	if err := decodeHashPairsExact(data2, &obj5); err != nil {
		t.Fatalf("decodeHashPairs failed: %v", err)
	}
	...
	if !hasOmitEmptyField(&obj3) || omitEmptyLen(&obj3) > 0 {
		...
		if n, err := decodeHashPairs(data4, &obj3); err != nil {
			t.Fatalf("decodeHashPairs failed: %v", err)
		} else if n != uint64(len(data2)) {
			t.Fatalf("decodeHashPairs bytes read length should be %d, is %d", len(data2), n)
		}
...
func decodeHashPairsExactExpectError(t *testing.T, buf []byte, expectedErr error) {
	var obj HashPairs
	if err := decodeHashPairsExact(buf, &obj); err == nil {
		t.Fatal("decodeHashPairsExact: expected error, got nil")
	} else if err != expectedErr {
		t.Fatalf("decodeHashPairsExact: expected error %q, got %q", expectedErr, err)
	}
}
...
func testSkyencoderHashPairsDecodeErrors(t *testing.T, k int, tag string, obj *HashPairs) {
	...
	if hasOmitEmptyField(obj) && numEncodableFields(obj) > 1 {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow nil", k, tag), func(t *testing.T) {
			decodeHashPairsExactExpectError(t, nil, encoder.ErrBufferUnderflow)
		})
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		t.Run(fmt.Sprintf("%d %s exact buffer underflow bytes=%d", k, tag, i), func(t *testing.T) {
			decodeHashPairsExactExpectError(t, buf[:i], encoder.ErrBufferUnderflow)
		})
	}
	...
	t.Run(fmt.Sprintf("%d %s exact buffer remaining bytes", k, tag), func(t *testing.T) {
		decodeHashPairsExactExpectError(t, buf, encoder.ErrRemainingBytes)
	})
}
