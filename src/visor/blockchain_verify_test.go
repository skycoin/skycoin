...
func TestVerifyTransactionSoftHardConstraints(t *testing.T) {
	...
	err = verifySingleTxnSoftHardConstraints(txn, params.MaxUserTransactionSize, params.CoinHourBurnFactor)
	require.NoError(t, err)

	// Transaction size exceeds maxSize
	err = verifySingleTxnSoftHardConstraints(txn, txn.Size()-1, params.CoinHourBurnFactor)
	...
	err = verifySingleTxnSoftHardConstraints(tx2, params.MaxUserTransactionSize, params.CoinHourBurnFactor)
	...
	err = verifySingleTxnSoftHardConstraints(lostCoinTx, params.MaxUserTransactionSize, params.CoinHourBurnFactor)
	...
	err = verifySingleTxnSoftHardConstraints(dupUxOutTx, params.MaxUserTransactionSize, params.CoinHourBurnFactor)
...
func TestVerifyTxnFeeCoinHoursAdditionFails(t *testing.T) {
	...
	err = VerifySingleTxnSoftConstraints(txn, head.Time()+1e6, uxIn, params.MaxUserTransactionSize, params.CoinHourBurnFactor)
...
func testVerifyTransactionAddressLocking(t *testing.T, toAddr string, expectedErr error) {
	...
	err = VerifySingleTxnSoftConstraints(txn, head.Time(), uxIn, params.MaxUserTransactionSize, params.CoinHourBurnFactor)
	if expectedErr == nil {
		require.NoError(t, err)
	} else {
