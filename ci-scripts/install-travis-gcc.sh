...
if [[ "$TRAVIS_OS_NAME" == "osx" ]]; then
  ...
  brew list gcc@6 &>/dev/null || (echo "Install gcc6 deps" && (brew deps -n gcc@6 | xargs -I NAME bash -c 'brew install NAME') && echo "Install gcc@6 from sources" && HOMEBREW_BUILD_FROM_SOURCE=1 brew install gcc@6 )
fi
