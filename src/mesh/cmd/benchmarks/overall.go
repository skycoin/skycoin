...
func main() {
	...
	clientAddr, serverAddr := clientConn.Address(), serverConn.Address()

	server := echoServer(serverConn)
...
func echoServer(serverConn messages.Connection) *app.Server {

	srv := app.NewServer(serverConn, func(in []byte) []byte {
		return in
	})
	return srv
}
