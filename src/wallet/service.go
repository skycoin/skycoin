...
func (serv *Service) CreateWallet(wltName string, options Options, tf TransactionsFinder) (Walleter, error) {
	serv.Lock()
	defer serv.Unlock()
	if !serv.config.EnableWalletAPI {
...
func (serv *Service) loadWallet(wltName string, options Options, tf TransactionsFinder) (Walleter, error) {
	...
	firstEntry := w.GetEntryAt(0)
	firstEntryAddress := firstEntry.Address.String()

	if _, ok := serv.firstAddrIDMap[firstEntryAddress]; ok {
	...
	serv.firstAddrIDMap[firstEntryAddress] = w.Filename()
...
func (serv *Service) EncryptWallet(wltID string, password []byte) (Walleter, error) {
	serv.Lock()
	defer serv.Unlock()
	if !serv.config.EnableWalletAPI {
		return nil, ErrWalletAPIDisabled
...
func (serv *Service) DecryptWallet(wltID string, password []byte) (Walleter, error) {
	serv.Lock()
	defer serv.Unlock()
	if !serv.config.EnableWalletAPI {
		return nil, ErrWalletAPIDisabled
...
func (serv *Service) GetWallet(wltID string) (Walleter, error) {
	serv.RLock()
	defer serv.RUnlock()
	if !serv.config.EnableWalletAPI {
		return nil, ErrWalletAPIDisabled
...
func (serv *Service) UnloadWallet(wltID string) error {
	...
	if wlt != nil && wlt.EntriesLen() > 0 {
		addr := wlt.GetEntryAt(0).Address.String()
...
func (serv *Service) GetWalletSeed(wltID string, password []byte) (string, error) {
	...
	var seed string
	if err := GuardView(w, password, func(wlt Walleter) error {
		seed = wlt.Seed()
		return nil
	}); err != nil {
		return "", err
...
func (serv *Service) UpdateSecrets(wltID string, password []byte, f func(Walleter) error) error {
	serv.Lock()
	defer serv.Unlock()
	if !serv.config.EnableWalletAPI {
		return ErrWalletAPIDisabled
...
func (serv *Service) Update(wltID string, f func(Walleter) error) error {
	serv.Lock()
	defer serv.Unlock()
	if !serv.config.EnableWalletAPI {
		return ErrWalletAPIDisabled
...
func (serv *Service) ViewSecrets(wltID string, password []byte, f func(Walleter) error) error {
	serv.RLock()
	defer serv.RUnlock()
	if !serv.config.EnableWalletAPI {
		return ErrWalletAPIDisabled
...
func (serv *Service) View(wltID string, f func(Walleter) error) error {
	serv.RLock()
	defer serv.RUnlock()
	if !serv.config.EnableWalletAPI {
		return ErrWalletAPIDisabled
...
func (serv *Service) RecoverWallet(wltName, seed string, password []byte) (Walleter, error) {
	serv.Lock()
	defer serv.Unlock()
	if !serv.config.EnableWalletAPI {
		return nil, ErrWalletAPIDisabled
