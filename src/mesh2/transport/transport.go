...
type Transport struct {
	...
	AckSent      uint32
	AckConfirmed uint32 // last confirmed ack
...
func (self *Transport) receiveIncoming() {
	...
	for msg := range self.IncomingChannel {
		...
		case messages.MsgOutRouteMessage:
			...
			m1b.Sequence = self.AckSent
			m1b.RouteId = m1.RouteId
...
func (self *Transport) receiveAck(msg []byte) {
	...
	if err != nil {
		...
		if lowestSequence > self.AckConfirmed {
			self.AckConfirmed = lowestSequence
		}
		fmt.Printf("transport %d sent %d packets and got %d acks\n", self.Id, self.AckSent, self.AckConfirmed)
	}
}
