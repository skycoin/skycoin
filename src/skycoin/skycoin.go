...
// ConfigureNote sets the note config values
func (c *Coin) ConfigureNote() note.Config {
	nc := note.NewConfig()

	nc.NotesDir = c.config.Node.NoteDirectory
	_, nc.EnableNoteAPI = c.config.Node.enabledAPISets[api.EndpointsNote]
