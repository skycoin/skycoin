...
func TestVerifyTransactionSoftHardConstraints(t *testing.T) {
	...
	err = verifySingleTxnSoftHardConstraints(txn, params.DefaultMaxBlockSize)
	require.NoError(t, err)
	...
	err = verifySingleTxnSoftHardConstraints(tx2, params.DefaultMaxBlockSize)
	...
	err = verifySingleTxnSoftHardConstraints(lostCoinTx, params.DefaultMaxBlockSize)
	...
	err = verifySingleTxnSoftHardConstraints(dupUxOutTx, params.DefaultMaxBlockSize)
...
func TestVerifyTxnFeeCoinHoursAdditionFails(t *testing.T) {
	...
	err = VerifySingleTxnSoftConstraints(txn, head.Time()+1e6, uxIn, params.DefaultMaxBlockSize)
...
func testVerifyTransactionAddressLocking(t *testing.T, toAddr string, expectedErr error) {
	...
	err = VerifySingleTxnSoftConstraints(txn, head.Time(), uxIn, params.DefaultMaxBlockSize)
	if expectedErr == nil {
		require.NoError(t, err)
	} else {
